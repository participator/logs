...
const server = http.createServer((req, res) => {
    ...
    if (fs.existsSync(requestedFile) && matchPublicFolder.test(req.url) || req.url === requestType.root) {
        ...
        if (req.url === requestType.root || req.url === requestType.home) {
            file = fs.createReadStream(getFileInCurrentDirectoryFullPath(requestType.home));
        }
        else if (req.url === requestType.app) {
    ...
    else if (req.url === requestType.login) {
        passport.authenticate('auth0', {
            scope: 'openid email profile'
        }, (req, res) => res.redirect(requestType.root));
    }
    else if (req.url === requestType.callback) {
        passport.authenticate('auth0', function (err, user, info) {
            console.log('[callback info]', info);
            if (err) { return next(err); }
            if (!user) { return res.redirect(requestType.login); }
            req.logIn(user, function (err) {
                if (err) { return next(err); }
                const returnTo = req.session.returnTo;
                delete req.session.returnTo;
                res.redirect(returnTo || requestType.root);
            });
        });
    }
    else if (req.url === requestType.logoff) {
